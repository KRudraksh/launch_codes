...
class FLIGHT_CONTROLLER:
	...
	def get_pose(self, location_data):
		x = location_data.pose.position.x
		y = location_data.pose.position.y
		z = location_data.pose.position.z
		new_pos = np.matmul(self.transformation_matrix,np.array([x,y,z]))
		self.pt.x = new_pos[0]
		self.pt.y = new_pos[1]
		self.pt.z = new_pos[2]
	...
	def gotopose(self,x,y,z):
		...
		sp.pose.position.x = new_pos[0]
		sp.pose.position.y = new_pos[1]
		sp.pose.position.z = new_pos[2]
