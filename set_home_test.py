...
class FLIGHT_CONTROLLER:
	...
	def __init__(self):
		...
		rospy.Subscriber('/mavros/global_position/global',NavSatFix, self.global_pos)
	...
	def sethome(self):
		...
		try:
			self.set_home_service(1, 0.0, self.gps_info.x, self.gps_info.y, mav.pt.z)
			rospy.loginfo("Home changed")

		except rospy.ServiceException as e:
			rospy.loginfo("Home could not be set: " %e)
...
# z -> altitude
	...
	def global_pos(self,data):
		self.gps_info.x = data.latitude
		self.gps_info.y = data.longitude
		self.gps_info.z = data.altitude
